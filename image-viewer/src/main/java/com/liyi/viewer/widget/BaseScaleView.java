...
public abstract class BaseScaleView extends FrameLayout {
    ...
    public BaseScaleView(@NonNull Context context) {
        super(context);
        initView(context);
    }

    public BaseScaleView(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        initView(context);
    }

    public BaseScaleView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void initView(Context context) {
        ...
        imageView = createPhotoView(context);
        imageView.setX(0);
        imageView.setY(0);
        ...
        addView(imageView.getImageView());
        initDragStateMonitor();
    }

    public abstract IPhotoView createPhotoView(Context context);
    /**
     * 初始化图片拖拽状态监测
     */
    ...
    public void setImageDraggerType(@ImageDraggerType int type, ImageViewerAttacher attacher, Drawable background) {
        ...
        if (mImageDragger != null) {
            ...
            if (attacher != null){
                mImageDragger.bindImageViewerAttacher(attacher);
            }
    ...
    public void recycle() {
        if (imageView != null) {
            ImageViewerUtil.recycleImage((ImageView)imageView);
        }
        clearImageDragger();
    }
    ...
    private void performExitAnim(final TransitionCallback callback) {
        ...
        final float to_x = new_width != 0 ?mViewData.getTargetX() : previewW / 2;
        final float to_y =new_height != 0 ?  mViewData.getTargetY() : previewH / 2;
        if (doBackgroundAlpha && mBackground == null && getBackground() != null) {
            mBackground = getBackground().mutate();
        }
